...
| batchSchedulerName | string | `nil` | batchSchedulerName, see flink-on-k8s-operator docs. |
...
| job.allowNonRestoredState | bool | `false` |  |
...
| job.savepointGeneration | string | `nil` |  |
| job.savepointsDir | string | `"/savepoint"` | (String) Directory to store automatically taken savepoints |
| job.takeSavepointOnUpgrade | bool | `false` | (bool) Should take savepoint before upgrading the job (TODO do we want to default to true?) |
