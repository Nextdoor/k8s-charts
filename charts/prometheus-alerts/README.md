...
| containerRules.AlertRulesSelectorsValidity | object | `{"for":"1h","resources":{"daemonsets":{"enabled":true},"deployments":{"enabled":true},"hpas":{"enabled":true},"jobs":{"enabled":true},"pods":{"enabled":true},"statefulsets":{"enabled":true}},"severity":"warning"}` | Does a basic lookup using the defined selectors to see if we can see any info for a given selector. This is the "watcher for the watcher". If we get alerted by this, we likely have a bad selector and our alerts are not going to ever fire. |
| containerRules.AlertRulesSelectorsValidity.resources | object | `{"daemonsets":{"enabled":true},"deployments":{"enabled":true},"hpas":{"enabled":true},"jobs":{"enabled":true},"pods":{"enabled":true},"statefulsets":{"enabled":true}}` | These flags let you disable this alarm for particular resources if you do not depoy that type. For example, if you do not deploy StatefulSets, only a Deployment, you can disable the StatefulSet check. |
